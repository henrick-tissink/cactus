...
- **Task 10 — `getByLabelText` does not work on `Transactions.tsx` filter labels.** The plan's verbatim test code used `getByLabelText`/`findByLabelText`/`queryByLabelText` for "Account"/"Status". The page uses bare `<label>` elements at `Transactions.tsx:253–289` with no `htmlFor`/`id` association to the inputs, so React Testing Library's accessible-label queries cannot match them. Test code adapted to `getByText`/`findByText`/`queryByText` (single matches; "Account" and "Status" are unique strings in the open-panel state). For a stronger long-term fix, `Transactions.tsx`'s filter labels should be wired with `htmlFor` (out of scope for PR 3; queue for an a11y cleanup PR).
- **Task 11 — `findByText(/emergency fund/i)` matches twice.** Not the modal-related collision the plan worried about. The seed goal `name: 'Emergency Fund'` coincidentally collides with `GOAL_TYPE_INFO[GoalType.EmergencyFund].label = 'Emergency Fund'` rendered at `Goals.tsx:334`. Tests switched to `findByRole('heading', { name: /emergency fund/i })` — sharper assertion (matches the `<h3>` only at line 331) and decoupled from the type-label table. Future tests on goal-name rendering should use the role-based query.
- **Task 11 — empty-state assertion needs a positive post-load anchor.** Initial implementation of test 2 (`waitFor(() => expect(queryAllByText(/emergency fund/i)).toHaveLength(0))`) would resolve immediately during the loading skeleton phase, never confirming the actual loaded-but-empty state. Fixed by anchoring on `await screen.findByText(/0 active goals?/i)` (matches `Goals.tsx:127` rendering `{activeGoals.length} active goal{...}`) before the negative assertion. Lesson for future page tests: any "absence" assertion needs a positive load-completed anchor first.
- **Task 11 — modal heading regex tightened.** Plan template used `/create.*goal/i` as a guess; actual heading at `Goals.tsx:477` is `Create New Goal`. Test uses the literal `/create new goal/i`. The trigger button label `New Goal` (`Goals.tsx:135`) matched the plan's guess; no change there.
- **Task 12 — `npm run lint` is broken on `main` independent of this PR.** 6 pre-existing errors (all `react-hooks/set-state-in-effect`) in `Login.tsx`, `Register.tsx`, `Settings.tsx`, `SpendingPlan.tsx`, `Transactions.tsx`, `VerifyEmail.tsx`. PR 3 added zero lint errors but cannot pass full-repo lint either. PR 4 (CI workflow) needs to either fix or quarantine these before lint becomes a merge gate.
- **Task 12 — coverage HTML report leaks into ESLint runs.** After `npm run test:coverage`, `coverage/lcov-report/*.js` produces 3 ESLint warnings. ESLint's ignore patterns in `eslint.config.js` should be widened (add `coverage/`) when PR 4 wires up CI lint, otherwise CI will produce noise.
- **Task 12 — `react-refresh/only-export-components` did not fire on `render.tsx`.** The plan defensively recommended an inline disable comment. Not needed in practice — the rule allows non-component named exports when at least one component is exported in the same file. Future test helpers that add only non-component exports may still need it.
- **Task 8 — Login error-path test logs benign jsdom navigation warning.** `apiClient.ts:55` does `window.location.href = '/login'` from the 401 interceptor; jsdom logs `Error: Not implemented: navigation`. Test passes; warning is cosmetic but could mask real warnings as the suite grows. Tracking item for a future test-hygiene pass — silence with a scoped `vi.spyOn(window.location, 'assign')` mock or refactor the redirect path.
- **Frontend coverage baseline measured: `src/pages/` 25.26% line.** Per-page: Login 98.72 · Dashboard 89.78 · Goals 51.93 · Transactions 36.44 · 9 untested pages at 0. PR 7 will lock the gate at `floor(25.26) − 2 = 23%` per the design doc's coverage-strategy section.
- **`MemoryRouter` over `BrowserRouter` is a hard requirement** for the render helper, not a stylistic preference. Tests that triggered the axios 401-redirect path under `BrowserRouter` would write to `window.location` and bleed across tests; `MemoryRouter` is in-memory and isolates routing state per render.
